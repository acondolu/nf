...
We are a bit cheating here: in fact, the notation for \AEQ{} hides a dependency on \coqdocdefinition{EQ}, which would actually make the \coqdockw{Fixpoint} definition fail as it conceals the \TODO{}. For the actual \Coq{} definition, we inlined \coqdocdefinition{eq\_aczel} so to make \Coq{} recognise the decreasing argument. However, we decided to just use the succint notation above, so that \TODO{} the intuition.

Proving that \coqdocdefinition{EQ} is an equivalence relation 
...
To conclude, we just need to show that the universal set cannot be low. 
