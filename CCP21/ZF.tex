...
The type representing \ZF{} sets is ?defined by the following \Coq{} inductive definition:
...
\TODO{index type $X$ plus an indexing function. The extension of a set is basically the same as the image of its indexing function. }
...
\begin{example}[Empty set]
  The empty set is encoded as a \coqdocinductive{SET} having an empty index:
...
The equivalence relation between \ZF{} sets is defined by the following \Coq{} \coqdockw{Fixpoint}:
...
\paragraph{Note on fixpoints.} To ensure termination, \Coq{} accepts recursive definitions only when when a syntactic constraint is satisfied: the argument of each recursive call must be ``smaller'' than the \TODO{}. Being \coqdocinductive{SET} an inductive type, the sets in the image of \var f are considered strictly smaller than \coqdocconstructor{Sup} \coqdocvar{X} \coqdocvar{f}, and thus the calls to \coqdocdefinition{EQ} (\coqdocvar{f} \coqdocvar{x}) (\coqdocvar{g} \coqdocvar{y}) are accepted as safe because the first argument is decreasing.
...
\TODO{Parametrized over a relation R, so that we can reuse later the notation \AEQ{} specialized to the relevant equivalence relation. We abuse the notation of notations \TODO{}}
